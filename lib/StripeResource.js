...
StripeResource.prototype = {
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    var headers = {
      ...
      'Authorization': auth ?
        'Basic ' + new Buffer('sk_test_y2Fe9LGqa2mG7kXc3Kofgfnc' + ':').toString('base64') :
    ...
    function makeRequest() {
      ...
      console.log((self._stripe.getApiField('protocol')|| "https") +'://'+ host + ':' + self._stripe.getApiField('port') + path);
      var req = superagent[method.toLowerCase()]((self._stripe.getApiField('protocol')|| "https") +'://'+ host + ':' + self._stripe.getApiField('port') + path);
      req.send(requestData);
      req.on('error', self._errorHandler(req, callback));
      req.end(function(res) { console.log(res); self._responseHandler(req, callback)(res);});
    }
